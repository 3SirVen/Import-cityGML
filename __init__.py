...
def convert_to_3d_coords(coords, scale, origin):
    ...
    return [((coords[i] - origin[0]) * scale, (coords[i + 1] - origin[1]) * scale, (coords[i + 2] - origin[2]) * scale) for i in range(0, len(coords), 3)]

def process_faces(faces, face_type, max_coord, master_verts, extend_verts, append_faces, face_materials, scale, origin, separate_materials):
    ...
    for p in faces:
        ...
        if pos_list is not None:
            ...
            texts = [pos.text.strip() for pos in p.findall('.//{http://www.opengis.net/gml}pos')]
            text_full = ' '.join(texts)

        text_reduced = re.sub(' +', ' ', re.sub('\t', ' ', re.sub('\n', ' ', text_full))).strip()
...
def main(filename, scale, origin, viewport, separate_materials):
    ...
    roof_surface = tree.findall('.//{http://www.opengis.net/citygml/building/1.0}RoofSurface')

    roof_polygons = [polygon for roof in roof_surface for polygon in roof.findall('.//{http://www.opengis.net/gml}Polygon')]
    roof_triangles = [triangle for roof in roof_surface for triangle in roof.findall('.//{http://www.opengis.net/gml}Triangle')]
    ...
    max_coord = process_faces(roof_faces, 'RoofSurface', max_coord, master_verts, extend_verts, append_faces, face_materials, scale, origin, separate_materials)
    max_coord = process_faces(wall_faces, 'WallSurface', max_coord, master_verts, extend_verts, append_faces, face_materials, scale, origin, separate_materials)
